#!/bin/bash
# Complete training pipeline for OrbPro Cesium SLM
# Usage: ./scripts/train-cesium-slm.sh [--size 0.5B|1.5B|3B] [--epochs N] [--skip-train] [--skip-compile]
...
case $MODEL_SIZE in
    0.5B|0.5b)
        ...
        LORA_DIR="cesium-qwen-lora-mlx"
        OUTPUT_NAME="OrbPro-Cesium-SLM-0.5B-${QUANTIZATION}-MLC"
        VRAM_EST="600MB"
        ;;
    1.5B|1.5b)
        BASE_MODEL="Qwen/Qwen2.5-1.5B-Instruct"
        LORA_DIR="cesium-qwen-1.5b-lora-mlx"
        OUTPUT_NAME="OrbPro-Cesium-SLM-1.5B-${QUANTIZATION}-MLC"
        VRAM_EST="1GB"
        ;;
    3B|3b)
        BASE_MODEL="Qwen/Qwen2.5-3B-Instruct"
        LORA_DIR="cesium-qwen-3b-lora-mlx"
        OUTPUT_NAME="OrbPro-Cesium-SLM-3B-${QUANTIZATION}-MLC"
        VRAM_EST="2GB"
        ;;
    *)
...
echo -e "${GREEN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║     OrbPro Cesium SLM - Complete Training Pipeline        ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ "$SKIP_COMPILE" = false ]; then
    ...
    COMPILE_SCRIPT="${SCRIPT_DIR}/compile-cesium-slm-docker.sh"

    # Create a temporary modified compile script
    TEMP_COMPILE=$(mktemp)
    sed -e "s|^MODEL_DIR=.*|MODEL_DIR=\"${MERGED_DIR}\"|" \
